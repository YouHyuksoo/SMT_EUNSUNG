...
const CONFIG_MAP: Record<SelectType, TypeConfig> = {
  line: {
    ...
    sectionTitleKey: 'lineSelect',
    allLabelKey: 'allLines',
  },
  sensor: {
    ...
    sectionTitleKey: 'sensorSelect',
    allLabelKey: 'allSensors',
  },
};
...
export default function DisplaySelectModal({
  ...
  /* ── 렌더 ── */
  return (
    <Modal isOpen={isOpen} onClose={handleClose} title={t('lineSettings')} size="md"
      footer={
        <>
          {!(required && !hasSaved) && (
            <Button variant="ghost" onClick={handleClose}>{tc('cancel')}</Button>
          )}
          <Button variant="primary" onClick={handleSave}>{t('apply')}</Button>
        </>
      }
    >
      {/* 타이밍 설정 */}
      <div className="mb-4 rounded-lg border border-zinc-200 p-4 dark:border-zinc-700">
        <h3 className="mb-3 text-sm font-bold text-zinc-900 dark:text-white">{t('timingSection')}</h3>
        <div className="flex flex-col gap-3">
          {/* 새로고침 간격 */}
          <div className="flex items-center gap-3">
            <label className="w-28 shrink-0 text-sm text-zinc-700 dark:text-zinc-300">{t('refreshLabel')}</label>
            <input
              type="range"
            ...
            <span className="w-16 text-right font-mono text-sm font-bold text-sky-400">
              {timing.refreshSeconds}{tc('seconds')}
            </span>
          </div>
          {/* 스크롤 간격 */}
          <div className="flex items-center gap-3">
            <label className="w-28 shrink-0 text-sm text-zinc-700 dark:text-zinc-300">{t('scrollLabel')}</label>
            <input
              type="range"
              min={3} max={60} step={1}
            ...
            />
            <span className="w-16 text-right font-mono text-sm font-bold text-sky-400">
              {timing.scrollSeconds}{tc('seconds')}
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-lg border border-zinc-200 p-4 dark:border-zinc-700">
        <h3 className="mb-3 text-sm font-bold text-zinc-900 dark:text-white">{t(config.sectionTitleKey)}</h3>
        {showWarning && (
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col gap-1">
            <label className="flex cursor-pointer items-center gap-3 rounded-lg border-b border-zinc-200 p-2 transition-colors hover:bg-zinc-100 dark:border-zinc-700 dark:hover:bg-zinc-800">
              ...
              <span className="font-bold text-zinc-900 dark:text-white">{t(config.allLabelKey)}</span>
            </label>

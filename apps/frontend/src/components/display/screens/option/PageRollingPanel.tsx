...
import { getLocalizedScreenTitle, SCREENS } from '@/lib/screens';
...
export default function PageRollingPanel() {
  ...
  const screenOptions = Object.values(SCREENS)
    .filter((s) => s.id !== '18')
    .map((s) => ({ id: s.id, label: `${getLocalizedScreenTitle(s, locale)} (${s.id})` }));
  ...
  const screenLabel = (id: string) =>
    screenOptions.find((s) => s.id === id)?.label ?? `${t('screenFallback')} ${id}`;
      ...
      <div>
        ...
        <div className="flex gap-2">
          ...
          >
            ...
            {screenOptions.filter((s) => !config.screens.includes(s.id)).map((s) => (

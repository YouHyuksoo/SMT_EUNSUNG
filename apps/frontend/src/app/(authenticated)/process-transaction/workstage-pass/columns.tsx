...
export const workstagePassColumns = (mode: WorkstagePassMode): ColumnDef<WorkstagePassRow>[] =>
  mode === 'inventory' ? [
    ...
    { accessorKey: 'workstageCode', header: '공정', size: 140, cell: ctx => ctx.row.original.workstageName ?? ctx.getValue() ?? '' }, { accessorKey: 'modelName', header: '모델명', size: 180 },
    { accessorKey: 'ioQty', header: '재공수량', size: 110, meta: { align: 'right' }, cell: c => qty(c.getValue()) },
  ] : mode === 'workstageSummary' ? [
    { accessorKey: 'workstageCode', header: '공정', size: 140, cell: ctx => ctx.row.original.workstageName ?? ctx.getValue() ?? '' }, { accessorKey: 'modelName', header: '모델명', size: 180 },
    { accessorKey: 'ioDeficit', header: '수불', size: 80, cell: comCodeCell('IO DEFICIT') }, { accessorKey: 'ioQty', header: '수량', size: 110, meta: { align: 'right' }, cell: c => qty(c.getValue()) },
  ] : detail;

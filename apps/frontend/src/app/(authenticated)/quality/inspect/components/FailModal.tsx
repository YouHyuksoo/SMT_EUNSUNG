...
/**
 * @file quality/inspect/components/FailModal.tsx
 * @description 외관검사 불합격 등록 모달 - 불량코드 + 상세 입력
 *
 * 초보자 가이드:
...
export default function FailModal({ isOpen, onClose, onSubmit, submitting }: Props) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose} title={t("quality.inspect.fail")} size="md">
      <div className="flex flex-col gap-4 p-4">
        ...
        <div>
          <label className="block text-sm font-medium text-text mb-1">
            {t("quality.inspect.detailReason")}
          </label>
          <textarea

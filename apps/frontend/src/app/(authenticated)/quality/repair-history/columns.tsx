...
/** 코드 셀 — 값이 없으면 배지를 그리지 않는다(빈 배지 노이즈 방지) */
function code(groupCode: string) {
  return function CodeCell(ctx: CellContext<RepairHistoryRow, unknown>) {
...
export const repairHistoryColumns: ColumnDef<RepairHistoryRow>[] = [
  ...
  { accessorKey: 'receiptDeficit', header: '불량구분', size: 100, cell: code('RECEIPT DEFICIT') },
  { accessorKey: 'qcInspectHandling', header: '검사처리', size: 100, cell: code('QC INSPECT HANDLING') },
  { accessorKey: 'qcResult', header: 'QC결과', size: 90, cell: code('QC RESULT') },
  ...
  { accessorKey: 'badReasonCode', header: '불량사유', size: 110, cell: code('BAD REASON CODE') },
  ...
  { accessorKey: 'repairResultCode', header: '수리결과', size: 100, cell: code('REPAIR RESULT CODE') },

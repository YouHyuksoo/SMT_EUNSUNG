...
summary: DB mapping toàn bộ cột master hạng mục, đồng bộ ERP (IF_ITEM_MASTER), phân quyền, xử lý sự cố
tags: [thông tin cơ sở, hạng mục, master, vận hành, ERP]
keywords: [ITEM_MASTERS, IF_ITEM_MASTER, ERP-IF, PRODUCT_TYPE, UNIT_TYPE, loại hạng mục, đồng bộ, đa khách hàng]
...
Màn hình quản lý **bảng master `ITEM_MASTERS`** chứa thông tin cơ sở của mọi hạng mục. BOM·kết quả sản xuất·nhập xuất vật tư·tồn kho đều tham chiếu master này qua `ITEM_CODE`.
...
- Mã chung: `PRODUCT_TYPE`, `UNIT_TYPE`, `USE_YN`
...
## Dữ liệu & Liên kết
- Bảng: `ITEM_MASTERS`
- Liên kết: BOM, Kết quả sản xuất, Nhập xuất vật tư, Tồn kho, Nhãn

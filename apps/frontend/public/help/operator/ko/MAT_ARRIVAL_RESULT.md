...
| **PO_ID(PO_ID)** | `MAT_ARRIVALS.PO_ID` | 해당 업무 단계의 식별, 상태, 수량 또는 추적 정보를 보관합니다. 운영 시 원본 업무와 수불 원장 대조에 사용합니다. |
| **PO_ITEM_ID(PO_ITEM_ID)** | `MAT_ARRIVALS.PO_ITEM_ID` | 해당 업무 단계의 식별, 상태, 수량 또는 추적 정보를 보관합니다. 운영 시 원본 업무와 수불 원장 대조에 사용합니다. |
| **INVOICE_NO(INVOICE_NO)** | `MAT_ARRIVALS.INVOICE_NO` | 해당 업무 단계의 식별, 상태, 수량 또는 추적 정보를 보관합니다. 운영 시 원본 업무와 수불 원장 대조에 사용합니다. |
| **VENDOR_ID(VENDOR_ID)** | `MAT_ARRIVALS.VENDOR_ID` | 해당 업무 단계의 식별, 상태, 수량 또는 추적 정보를 보관합니다. 운영 시 원본 업무와 수불 원장 대조에 사용합니다. |
...
| **WAREHOUSE_CODE(WAREHOUSE_CODE)** | `MAT_ARRIVALS.WAREHOUSE_CODE` | 창고 기준정보 코드입니다. |
| **ARRIVAL_DATE(ARRIVAL_DATE)** | `MAT_ARRIVALS.ARRIVAL_DATE` | 해당 업무 단계의 식별, 상태, 수량 또는 추적 정보를 보관합니다. 운영 시 원본 업무와 수불 원장 대조에 사용합니다. |
| **ARRIVAL_TYPE(ARRIVAL_TYPE)** | `MAT_ARRIVALS.ARRIVAL_TYPE` | 해당 업무 단계의 식별, 상태, 수량 또는 추적 정보를 보관합니다. 운영 시 원본 업무와 수불 원장 대조에 사용합니다. |
| **WORKER_NO(WORKER_NO)** | `MAT_ARRIVALS.WORKER_NO` | 작업자 사번/사용자 식별값입니다. |
| **IQC_STATUS(IQC_STATUS)** | `MAT_ARRIVALS.IQC_STATUS` | 수입검사 진행 또는 판정 상태입니다. |
| **STATUS(STATUS)** | `MAT_ARRIVALS.STATUS` | 업무 행의 현재 상태입니다. |
| **REMARK(REMARK)** | `MAT_ARRIVALS.REMARK` | 업무 메모 또는 취소/처리 사유입니다. |
...
| **CREATED_BY(CREATED_BY)** | `MAT_ARRIVALS.CREATED_BY` | 생성 사용자입니다. |
| **UPDATED_BY(UPDATED_BY)** | `MAT_ARRIVALS.UPDATED_BY` | 수정 사용자입니다. |
| **CREATED_AT(CREATED_AT)** | `MAT_ARRIVALS.CREATED_AT` | 레코드 생성 시각입니다. |
| **UPDATED_AT(UPDATED_AT)** | `MAT_ARRIVALS.UPDATED_AT` | 마지막 수정 시각입니다. |
...
| **COMPANY(COMPANY)** | `MAT_ARRIVAL_STOCKS.COMPANY` | 회사 스코프입니다. 다른 회사 데이터와 섞이지 않게 하는 필수 조건입니다. |
| **PLANT_CD(PLANT_CD)** | `MAT_ARRIVAL_STOCKS.PLANT_CD` | 사업장 스코프입니다. 공장별 자재 흐름을 분리합니다. |
| **MAT_UID(MAT_UID)** | `MAT_ARRIVAL_STOCKS.MAT_UID` | 자재 LOT/시리얼의 고유 식별자입니다. |
| **ARRIVAL_NO(ARRIVAL_NO)** | `MAT_ARRIVAL_STOCKS.ARRIVAL_NO` | 입하 업무 번호입니다. |
| **ARRIVAL_SEQ(ARRIVAL_SEQ)** | `MAT_ARRIVAL_STOCKS.ARRIVAL_SEQ` | 입하번호 내 상세 순번입니다. |
| **WAREHOUSE_CODE(WAREHOUSE_CODE)** | `MAT_ARRIVAL_STOCKS.WAREHOUSE_CODE` | 창고 기준정보 코드입니다. |
| **ITEM_CODE(ITEM_CODE)** | `MAT_ARRIVAL_STOCKS.ITEM_CODE` | 품목마스터 기준 품목코드입니다. |
| **QTY(QTY)** | `MAT_ARRIVAL_STOCKS.QTY` | 업무 처리 수량입니다. |
| **AVAILABLE_QTY(AVAILABLE_QTY)** | `MAT_ARRIVAL_STOCKS.AVAILABLE_QTY` | 예약·보류를 제외하고 처리 가능한 수량입니다. |
| **STATUS(STATUS)** | `MAT_ARRIVAL_STOCKS.STATUS` | 업무 행의 현재 상태입니다. |
| **CREATED_BY(CREATED_BY)** | `MAT_ARRIVAL_STOCKS.CREATED_BY` | 생성 사용자입니다. |
| **UPDATED_BY(UPDATED_BY)** | `MAT_ARRIVAL_STOCKS.UPDATED_BY` | 수정 사용자입니다. |
| **CREATED_AT(CREATED_AT)** | `MAT_ARRIVAL_STOCKS.CREATED_AT` | 레코드 생성 시각입니다. |
| **UPDATED_AT(UPDATED_AT)** | `MAT_ARRIVAL_STOCKS.UPDATED_AT` | 마지막 수정 시각입니다. |
...
| 구분 | 내용 |
|------|------|
| 주요 테이블 | `MAT_ARRIVALS`, `MAT_LOTS`, `MAT_ARRIVAL_STOCKS` |
| 연계 화면 | [MAT_ARRIVAL, MAT_ARRIVAL_TRANSACTION, INV_ARRIVAL_STOCK] |

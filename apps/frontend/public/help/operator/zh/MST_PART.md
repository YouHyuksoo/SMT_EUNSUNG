...
audience: operator
title: 物料主表 — 操作指南
summary: 物料主表全部列的DB映射, ERP同步(IF_ITEM_MASTER), 权限, 故障排除
tags: [标准信息, 物料, 主表, 操作, ERP]
keywords: [ITEM_MASTERS, IF_ITEM_MASTER, ERP-IF, PRODUCT_TYPE, UNIT_TYPE, 物料类型, 同步, 多租户]
...
## 系统目的·角色
持有所有物料标准信息的**主表 `ITEM_MASTERS`** 管理界面。BOM·生产实绩·材料收发·库存均通过 `ITEM_CODE` 引用此主表。
...
## 预设条件 (主表·公共代码)
- 公共代码: `PRODUCT_TYPE`, `UNIT_TYPE`, `USE_YN`

...
export class MatIssueController {
  ...
  async findAll(@Query() query: MatIssueQueryDto, @OrganizationId() organizationId: number) {
    const result = await this.matIssueService.findAll(query, organizationId);
  ...
  async scanIssue(@Body() dto: ScanIssueDto, @OrganizationId() organizationId: number) {
    const data = await this.matIssueService.scanIssue(dto, organizationId);
    return ResponseUtil.success(data, '스캔 출고가 완료되었습니다.');
  }

  @Get(':issueNo/:seq')
  @ApiOperation({ summary: '출고 상세 조회' })
  async findById(@Param('issueNo') issueNo: string, @Param('seq') seq: string, @OrganizationId() organizationId: number) {
    const data = await this.matIssueService.findById(issueNo, Number(seq), organizationId);
  ...
  async create(@Body() dto: CreateMatIssueDto, @OrganizationId() organizationId: number) {
    const data = await this.matIssueService.create(dto, organizationId);
  ...
  async cancel(@Param('issueNo') issueNo: string, @Param('seq') seq: string, @Body('reason') reason: string | undefined, @OrganizationId() organizationId: number) {
    const data = await this.matIssueService.cancel(issueNo, Number(seq), reason, organizationId);

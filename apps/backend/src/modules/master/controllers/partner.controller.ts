...
export class PartnerController {
  ...
  async getStatistics(@OrganizationId() organizationId: number) {
    const data = await this.partnerService.getStatistics(organizationId);
  ...
  async findByType(@Param('type') type: string, @OrganizationId() organizationId: number) {
    const data = await this.partnerService.findByType(type, organizationId);
    return ResponseUtil.success(data);
  }
  ...
  async findByCode(
    ...
    const data = await this.partnerService.findByCode(partnerCode, organizationId);
    return ResponseUtil.success(data);
  }

  @Get()
  @ApiOperation({ summary: '거래처 목록 조회' })
  async findAll(@Query() query: PartnerQueryDto, @OrganizationId() organizationId: number) {
    const result = await this.partnerService.findAll(query, organizationId);
  ...
  async findById(@Param('id') id: string, @OrganizationId() organizationId: number) {
    const data = await this.partnerService.findById(id, organizationId);
    return ResponseUtil.success(data);
  }
  ...
  async create(@Body() dto: CreatePartnerDto, @OrganizationId() organizationId: number) {
    const data = await this.partnerService.create(dto, organizationId);
  ...
  async update(
    ...
    const data = await this.partnerService.update(id, dto, organizationId);
  ...
  async delete(@Param('id') id: string, @OrganizationId() organizationId: number) {
    await this.partnerService.delete(id, organizationId);

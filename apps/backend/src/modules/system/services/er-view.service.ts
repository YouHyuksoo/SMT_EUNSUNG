  ...
  private regenerateErd() {
    try {
      execFileSync('python', ['tools/generate_db_schema_doc.py'], {
        cwd: process.cwd(),
        env: { ...process.env, ORACLE_SITE: process.env.ORACLE_SITE ?? 'ES_JSIDC' },
        stdio: 'pipe',
      });
      return { status: 'SUCCESS' };
    } catch (error) {
      return { status: 'FAILED', errorMessage: String(error), command: 'ORACLE_SITE=ES_JSIDC python tools/generate_db_schema_doc.py' };
    }
  }

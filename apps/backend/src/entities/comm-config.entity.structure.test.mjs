...
const migrationSource = readFileSync(resolve(repoRoot, "apps/backend/src/migrations/2026-07-08_create_comm_configs.sql"), "utf8");

test("COMM_CONFIGS migration matches the current organization-scoped entity shape", () => {
  assert.match(entitySource, /@Entity\(\{ name: 'COMM_CONFIGS' \}\)/);
  assert.match(entitySource, /name: 'ORGANIZATION_ID'/);

  assert.match(migrationSource, /CREATE TABLE COMM_CONFIGS/);
  ...
  assert.match(migrationSource, /PK_COMM_CONFIGS/);
});

-- 통합검사 메뉴 등록 (INSP_INTEGRATED)
-- 검사관리(INSPECTION) 카테고리 첫 번째(3)에 배치.
-- 경로/라벨은 프론트 menuConfig(INSP_INTEGRATED -> /inspection/integrated, menu.inspection.integrated).
...
INSERT ALL
  WHEN menu_missing = 1 THEN
    ...
    VALUES ('INSP_INTEGRATED', 'INSPECTION', 3, '40', '1000', 'system', 'system')
  WHEN manager_missing = 1 THEN
    INTO ROLE_MENU_PERMISSIONS (ROLE_CODE, MENU_CODE, CAN_ACCESS, COMPANY, PLANT_CD)
    VALUES ('MANAGER', 'INSP_INTEGRATED', 'Y', '40', '1000')
  WHEN operator_missing = 1 THEN
    INTO ROLE_MENU_PERMISSIONS (ROLE_CODE, MENU_CODE, CAN_ACCESS, COMPANY, PLANT_CD)
    VALUES ('OPERATOR', 'INSP_INTEGRATED', 'Y', '40', '1000')
SELECT
  CASE WHEN NOT EXISTS (
    SELECT 1 FROM MENU_CATEGORY_ITEMS
    WHERE MENU_CODE = 'INSP_INTEGRATED'
      AND COMPANY = '40'
      AND PLANT_CD = '1000'

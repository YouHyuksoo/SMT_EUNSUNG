CREATE OR REPLACE PROCEDURE "P_INSERT_NPM3_RAW" (
  /* ================================================================
   * 프로시저명  : P_INSERT_NPM3_RAW
   * 작성자  : 지성솔루션컨설팅
   * 작성일  : 2020-07-30
   * 수정이력:
   ...
   *   IQ_MACHINE_INSPECT_NPM3_RAW - 원본 로직 참조 테이블
   * ================================================================
   * [AI 분석] 호출 객체:
   *   P_DATA
   *   P_INFO
...
BEGIN
  ...
  if p_info.Last >= 4
    ...
    if LVI_START_INDEX > 0 and LVI_END_INDEX > 0 
     ...
     INSERT INTO IQ_MACHINE_INSPECT_NPM3_RAW
      ...
      (
        ...
        LVS_PATH_NAME, --REPLACE(TRIM(p_data(14)),'"'),
...
EXCEPTION
   ...
   WHEN OTHERS
   THEN
      LVS_ERRORMSG := '[P_INSERT_NPM3_RAW]' ||  SUBSTR(SQLERRM,1,200)  ;

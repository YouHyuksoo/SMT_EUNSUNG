CREATE OR REPLACE PROCEDURE P_INTERLOCK_RESET_LINE (
  /* ================================================================
   * 프로시저명  : P_INTERLOCK_RESET_LINE
   * 작성자  : 지성솔루션컨설팅
   * 작성일  : 2020-07-30
   * 수정이력:
             ...
             A.NEW_SCAN_QTY = (
                               SELECT SUM(NVL (B.FEEDING_QTY, 0) - NVL (B.PRODUCT_ACTUAL_QTY, 0))
                                ...
                                WHERE B.line_code = SUBSTR (p_line_code, 1, 2)
                                  AND B.model_name = p_feeder_layout_name
                                  AND B.active_yn = 'Y'
   ...
   BEGIN
      ...
      UPDATE ip_product_line
         SET MASK_LOT_NO = NULL,
             ...
             LCR_CHECK_DATE = NULL ,
             LCR_LOT_NO = NULL,
             NOZZLE_CHECK_DATE = NULL,
             NOZZLE_CHECK_STATUS = NULL
       WHERE line_code = SUBSTR (p_line_code, 1, 2);

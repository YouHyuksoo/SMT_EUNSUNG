  ...
  IF v_count = 0 THEN
    EXECUTE IMMEDIATE q'[
      CREATE TABLE OEE_OPERATION_LOG (
        ...
        CONSTRAINT CK_OEE_OPLOG_SHIFT CHECK (SHIFT IN ('DAY','NIGHT'))
      )
    ]';
  END IF;
END;
  ...
  IF v_count = 0 THEN
    EXECUTE IMMEDIATE q'[
      CREATE TABLE OEE_PLAN_TIME (
        ...
        CONSTRAINT CK_OEE_PLANTIME_SHIFT CHECK (SHIFT IN ('DAY','NIGHT'))
      )
    ]';
  END IF;
END;
  ...
  IF v_count = 0 THEN
    EXECUTE IMMEDIATE q'[
      CREATE TABLE OEE_DAILY_SUMMARY (
        ...
        CONSTRAINT CK_OEE_SUMMARY_SHIFT CHECK (SHIFT IN ('DAY','NIGHT'))
      )
    ]';
  END IF;
END;

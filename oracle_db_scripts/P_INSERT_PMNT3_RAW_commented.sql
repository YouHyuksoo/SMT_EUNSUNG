CREATE OR REPLACE PROCEDURE "P_INSERT_PMNT3_RAW" (
  /* ================================================================
   * 프로시저명  : P_INSERT_PMNT3_RAW
   * 작성자  : 지성솔루션컨설팅
   * 작성일  : 2020-07-30
   * 수정이력:
   ...
   *   IQ_MACHINE_INSPECT_PMNT3 - 원본 로직 참조 테이블
   * ================================================================
   * [AI 분석] 호출 객체:
   *   P_DATA
   *   P_INFO
...
IS
     ...
     LVS_MACHINE_TYPE     VARCHAR2(20); --CM602-2 -- [AI] 내부 처리용 변수
BEGIN
      ...
      if LVI_START_PRODUCT > 0 
        ...
        IF SUBSTR(LVS_MACHINE_TYPE,1,2) <> 'CM' THEN
          begin 
               ...
               LVS_ERRORMSG := '[P_INSERT_PMNT3_RAW] ' || 'SUBSTR(LVS_MACHINE_TYPE,1,2) <> CM' ;
  ...
  if p_data.Last >= 71 --Date= 
    ...
    LVS_LOG_DATE :=  TRIM(p_data(71));
     ...
     INSERT INTO IQ_MACHINE_INSPECT_PMNT3
...
EXCEPTION
   ...
   WHEN OTHERS
   THEN
      LVS_ERRORMSG := '[P_INSERT_PMNT3_RAW]' ||  SUBSTR(SQLERRM,1,200)  ;
     --  INSERT INTO ICOM_MACHINE_INSERT_LOG( LOG_DATE , ERROR_MESSAGE , ERROR_DESC ) 
     --  VALUES ( SYSDATE  , LVS_ERRORMSG  , lvs_file_name||' LINE='||lvs_line_code  ) ;
     -- COMMIT ;
      NULL;

END P_INSERT_PMNT3_RAW;
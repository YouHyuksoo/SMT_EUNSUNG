CREATE OR REPLACE TRIGGER "INFINITY21_JSMES"
   ...
   * ================================================================
   * [AI 분석] 기능 설명:
   *   IQ_MACHINE_INSPECT_PMNT4 테이블의 INSERT 이벤트 발생 시 원본 트리거 로직에 정의된 후속 처리를 자동 수행한다.
   ...
   * ================================================================
   * [AI 분석] 대상 객체:
   *   IQ_MACHINE_INSPECT_PMNT4 - 트리거가 걸린 테이블/뷰
   * ================================================================
   * [AI 분석] OLD/NEW 사용:
   *   :NEW.FEEDERSERIAL - 신규/변경 후 값 값
   *   :NEW.FILE_NAME - 신규/변경 후 명칭 관련 값
   *   :NEW.LINE_CODE - 신규/변경 후 라인 관련 값
   *   :NEW.MACHINE_CODE - 신규/변경 후 설비 관련 값
   *   :NEW.FADD - 신규/변경 후 값 값
   *   :NEW.FSADD - 신규/변경 후 값 값
   *   :NEW.PARTSNAME - 신규/변경 후 값 값
   *   :NEW.MCNO - 신규/변경 후 값 값
   *   :NEW.PICKUP - 신규/변경 후 값 값
   *   :NEW.PMISS - 신규/변경 후 값 값
   *   :NEW.RMISS - 신규/변경 후 값 값
   * ================================================================
   * [AI 분석] 참조 테이블:
   *   IQ_MACHINE_INSPECT_PMNT4 - 설비 / 검사 관련 트리거 대상 테이블
   ...
   * ================================================================
   * 검증 방법:
   *   SELECT status FROM user_objects WHERE object_type = 'TRIGGER' AND object_name = 'TR_IQ_MACHINE_PMNT4_INS';
   *   SELECT * FROM user_errors WHERE type = 'TRIGGER' AND name = 'TR_IQ_MACHINE_PMNT4_INS';
   *   주의: 검증 목적으로 대상 테이블에 DML을 실행하지 않는다.
   * ================================================================ */
."TR_IQ_MACHINE_PMNT4_INS" 

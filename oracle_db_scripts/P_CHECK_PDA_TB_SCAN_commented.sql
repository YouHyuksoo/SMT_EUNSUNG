CREATE OR REPLACE PROCEDURE P_CHECK_PDA_TB_SCAN (
  /* ================================================================
   * 프로시저명  : P_CHECK_PDA_TB_SCAN
   * 작성자  : 지성솔루션컨설팅
   * 작성일  : 2016-10-17
   * 수정이력:
...
IS
   ...
   lvs_item_code_supplier_replace   VARCHAR2 (30); -- [AI] 내부 처리용 변수
   ...
   lvl_pos1                         NUMBER; -- [AI] 내부 처리용 변수
   lvl_pos2                         NUMBER; -- [AI] 내부 처리용 변수
   lvl_pos3                         NUMBER; -- [AI] 내부 처리용 변수
...
BEGIN
   ...
   ----------------------------------------------------------------------------------------------       
       ...
       IF ( lvi_row_count = 0 ) THEN
           ...
           p_return := f_msg('[CCS] 출고된 자재 바코드가 아닙니다.', 'K', 1) ; 
           p_sequence := '' ;
           RETURN;

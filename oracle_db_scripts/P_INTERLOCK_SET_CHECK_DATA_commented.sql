CREATE OR REPLACE PROCEDURE "P_INTERLOCK_SET_CHECK_DATA" (
  /* ================================================================
   * 프로시저명  : P_INTERLOCK_SET_CHECK_DATA
   * 작성자  : 지성솔루션컨설팅
   * 작성일  : 2016-04-27
   * 수정이력:
   *   2016-04-27 - 지성솔루션컨설팅 - 최초 작성
   ...
   *   P_MAPPING_LABEL - 원본 선언부 기준 입력/출력 파라미터
   *   P_ATTRIBUTE1 - 원본 선언부 기준 입력/출력 파라미터
   *   P_ATTRIBUTE2 - 원본 선언부 기준 입력/출력 파라미터
   *   P_ATTRIBUTE3 - 원본 선언부 기준 입력/출력 파라미터
   *   P_OUT - 원본 선언부 기준 입력/출력 파라미터
   * ================================================================
   * [AI 분석] 참조 테이블:
   *   IP_PRODUCT_2D_BARCODE - 원본 로직 참조 테이블
   *   IP_PRODUCT_LINE - Product Line Master
   *   IP_PRODUCT_MAGAZINE - 원본 로직 참조 테이블
   *   IP_PRODUCT_MODEL_MASTER - 원본 로직 참조 테이블
   *   IP_PRODUCT_REINPUT_RESULT - 원본 로직 참조 테이블
   *   IP_PRODUCT_RUN_MODEL - 원본 로직 참조 테이블
   *   IP_PRODUCT_SERIAL_RESULT - 원본 로직 참조 테이블
   *   IQ_INTERLOCK_CHECK_RESULT - 원본 로직 참조 테이블
   * ================================================================
   * [AI 분석] 호출 객체:
   *   F_CHECK_MASTER_SAMPLE
   *   F_GET_MAGAZINE_UNISSUED_CUR
   *   F_GET_WORKSTAGE_TYPE
   *   F_GET_WORKTIME_ZONE
   *   F_GET_WORK_ACTUAL_DATE
   *   F_GET_WORK_SHIFT_CODE
   *   P_INTERLOCK_SENSOR_ACTUAL_NEO
   * ================================================================
   * [AI 분석] 예외 처리:
   ...
   * ================================================================
   * 사용 예시:
   *   EXEC P_INTERLOCK_SET_CHECK_DATA(...)
   * ================================================================ */
...
-------------------------------------------------------------------------
IS
   lvs_set_item_code         VARCHAR2 (30); -- [AI] 내부 처리용 변수
   lvs_run_no                VARCHAR2 (30); -- [AI] 내부 처리용 변수
   lvs_model_name            VARCHAR2 (30); -- [AI] 내부 처리용 변수
   lvs_pcb_item              VARCHAR2 (30); -- [AI] 내부 처리용 변수
   ...
   PHASE                     VARCHAR2 (1000); -- [AI] 내부 처리용 변수
   LVS_ERRORMSG              VARCHAR2 (1000); -- [AI] 내부 처리용 변수
   lvi_exists                NUMBER; -- [AI] 내부 처리용 변수
   lvi_carrier_size          NUMBER; -- [AI] 내부 처리용 변수
   lvs_out                   VARCHAR2 (1000); -- [AI] 내부 처리용 변수
   LVI_DUP_CHECK             NUMBER; -- [AI] 내부 처리용 변수
   LVD_SYSDATE               DATE; -- [AI] 내부 처리용 변수
   -- JIG 사용 Logging 2016.04.27
   LVS_MASK_LOT_NO           VARCHAR2 (50); -- [AI] 내부 처리용 변수
   LVS_SOLDER_LOT_NO         VARCHAR2 (50); -- [AI] 내부 처리용 변수
   LVS_SQUEZE_LOT_NO         VARCHAR2 (50); -- [AI] 내부 처리용 변수

   LVS_MAPPING_MODEL_NAME    VARCHAR2 (30); -- [AI] 내부 처리용 변수
   LVI_POS1                  NUMBER; -- [AI] 내부 처리용 변수
   LVI_POS2                  NUMBER; -- [AI] 내부 처리용 변수

   LVI_MAGAZINE_IN_QTY       NUMBER; -- [AI] 내부 처리용 변수

CREATE OR REPLACE PROCEDURE "P_CHECK_REEL_TB_SCAN_BAK" (
  /* ================================================================
   * 프로시저명  : P_CHECK_REEL_TB_SCAN_BAK
   * 작성자  : 지성솔루션컨설팅
   * 작성일  : 2016-10-17
   * 수정이력:
   ...
   ELSE
      ...
      BEGIN
         ...
         SELECT COUNT (*)
          ...
          WHERE lot_name      = UPPER (p_model_name)
            AND line_code     = SUBSTR (p_line_code, 1, 2)
            AND location_code = p_address
            AND pcb_item      = p_topbot
      ...
      IF NVL (lvi_last_lot_no, 0) = 0 THEN
         ...
         UPDATE ib_product_plandata
            SET check_status  = 'E',
                ...
                change_date   = SYSDATE
          WHERE line_code     = SUBSTR (p_line_code, 1, 2)
            AND model_name    = UPPER (p_model_name) --   AND machine = TRIM (SUBSTR (p_line_code, 4, 10))
            AND location_code = p_address
            AND active_yn     = 'Y'
            AND pcb_item      = p_topbot;

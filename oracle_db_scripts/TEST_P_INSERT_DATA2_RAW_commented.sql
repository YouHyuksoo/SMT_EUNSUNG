CREATE OR REPLACE PROCEDURE TEST_P_INSERT_DATA2_RAW (
  /* ================================================================
   * 프로시저명  : TEST_P_INSERT_DATA2_RAW
   * 작성자  : 지성솔루션컨설팅
   * 작성일  : 2020-07-30
   * 수정이력:
...
BEGIN
   ...
   FOR i IN p_data.LAST + 1 .. p_data.LAST + p_data2.LAST
   LOOP
      lvs_sql := lvs_sql || ', C' || i;
   END LOOP;
   ...
   FOR j IN p_data2.FIRST .. p_data2.LAST
   LOOP
      lvs_sql := lvs_sql || ', ''' || REPLACE (TRIM (p_data2 (j)), '"') || '''';
   END LOOP;   
...
EXCEPTION
   ...
   THEN
      LVS_ERRORMSG :=
         '[TEST_P_INSERT_DATA2_RAW]'|| SUBSTR (SQLERRM, 1, 200)|| lvs_sql;
    ...
    --   INSERT INTO ICOM_MACHINE_INSERT_LOG( LOG_DATE , ERROR_MESSAGE , ERROR_DESC )
     --  VALUES ( SYSDATE  , LVS_ERRORMSG  , lvs_file_name||' LINE='||lvs_line_code  ) ;
      ps_job_errorlog(990,1,'TEST_P_INSERT_DATA2_RAW','RAW',LVS_ERRORMSG,'FFF');
      COMMIT;
      NULL;
END TEST_P_INSERT_DATA2_RAW;

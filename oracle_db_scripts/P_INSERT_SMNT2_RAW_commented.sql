CREATE OR REPLACE PROCEDURE "P_INSERT_SMNT2_RAW" (
  /* ================================================================
   * 프로시저명  : P_INSERT_SMNT2_RAW
   * 작성자  : 지성솔루션컨설팅
   * 작성일  : 2020-07-30
   * 수정이력:
   ...
   * ================================================================
   * [AI 분석] 기능 설명:
   *   SMT 장비 SMNT2 원시 검사 데이터를 배열 파라미터에서 읽어 저장한다.
   *   파일명, 라인, 설비, 모델 정보를 p_info/p_data에서 추출한다.
   *   IQ_MACHINE_INSPECT_SMNT2에 피더/슬롯/자재/에러 관련 원시 데이터를 등록한다.
   * ================================================================
   * [AI 분석] 파라미터:
   ...
   * ================================================================
   * [AI 분석] 참조 테이블:
   *   IQ_MACHINE_INSPECT_SMNT2 - SMNT2 장비 원시 검사 데이터 테이블
   ...
   * ================================================================
   * 사용 예시:
   *   EXEC P_INSERT_SMNT2_RAW(:P_DATA, :P_INFO)
   * ================================================================ */
...
BEGIN
  ...
  if p_data.Last >= 9 --Model=
     then
    LVS_MODEL :=  TRIM(p_data(9));
  end if;

     INSERT INTO IQ_MACHINE_INSPECT_SMNT2
...
EXCEPTION
   ...
   THEN
      ...
      LVS_ERRORMSG := '[P_INSERT_SMNT2_RAW]' ||  SUBSTR(SQLERRM,1,200)  ;

CREATE OR REPLACE PROCEDURE P_INTERLOCK_CHECK_SHS
  /* ================================================================
   * 프로시저명  : P_INTERLOCK_CHECK_SHS
   * 작성자  : 지성솔루션컨설팅
   * 작성일  : 2021-10-27
   * 수정이력:
   *   2021-10-27 - 지성솔루션컨설팅 - 최초 작성
   ...
   *   P_OK_MESSAGE - 원본 선언부 기준 입력/출력 파라미터
   * ================================================================
   * [AI 분석] 참조 테이블:
   ...
   *   IP_PRODUCT_WORK_QC - 원본 로직 참조 테이블
   *   IQ_INTERLOCK_CHECK_CONDITION - 원본 로직 참조 테이블
   *   IQ_MACHINE_INSPECT_DATA_AOI - 원본 로직 참조 테이블
   *   IQ_MACHINE_INSPECT_DATA_ICT - 원본 로직 참조 테이블
   *   IQ_MACHINE_INSPECT_DATA_MK - 원본 로직 참조 테이블
   *   IQ_MACHINE_INSPECT_DATA_REFLOW - 원본 로직 참조 테이블
   *   IQ_MACHINE_INSPECT_DATA_RW - 원본 로직 참조 테이블
   *   IQ_MACHINE_INSPECT_DATA_SPI - 원본 로직 참조 테이블
   *   IQ_PRODUCT_WQC - 원본 로직 참조 테이블
   *   ISYS_CONFIG - 원본 로직 참조 테이블
   * ================================================================
   * [AI 분석] 호출 객체:
   *   FOUND
   *   F_GET_BASECODE
   *   F_GET_MODEL_DIVISTION
   *   F_GET_RUN_LINE_INPUT_QTY
   *   F_MSG
   *   P_INTERLOCK_CHECK_NG_LOG
   *   P_INTERLOCK_REQUEST_LOG
   * ================================================================
   * [AI 분석] 예외 처리:
...
IS
    ...
    lvs_data                    VARCHAR2 (100); -- [AI] 내부 처리용 변수
    lvs_message                 VARCHAR2 (1000); -- [AI] 내부 처리용 변수
    lvs_ok_message              VARCHAR2 (1000); -- [AI] 내부 처리용 변수
    lvs_jig_model_name          VARCHAR2 (30); -- [AI] 내부 처리용 변수
    lvs_jig_type                VARCHAR2 (10); -- [AI] 내부 처리용 변수
    lvs_jig_code                VARCHAR2 (30); -- [AI] 내부 처리용 변수
    lvs_jog_lot_no              VARCHAR2 (30); -- [AI] 내부 처리용 변수
    lvs_reflow_result           VARCHAR2 (10); -- [AI] 내부 처리용 변수
    lvs_solder_type             VARCHAR2 (10); -- [AI] 내부 처리용 변수
    lvs_solder_type_scan        VARCHAR2 (10); -- [AI] 내부 처리용 변수
    lvs_solder_barcode          VARCHAR2 (50); -- [AI] 내부 처리용 변수
    lvl_air_density             NUMBER; -- [AI] 내부 처리용 변수
    lvl_check_temp              VARCHAR2 (10); --NUMBER; -- [AI] 내부 처리용 변수
    lvs_serial_solder_type      VARCHAR2 (10); -- [AI] 내부 처리용 변수
    lvs_check_result            VARCHAR2 (100); -- [AI] 내부 처리용 변수
    ...
    LVDT_REPAIR_DATE            DATE; -- [AI] 내부 처리용 변수
    LVS_CARRIER_PID             VARCHAR2 (100); -- [AI] 내부 처리용 변수
    lvs_model_name              VARCHAR2 (30); -- [AI] 내부 처리용 변수
    lvs_model_division          VARCHAR2 (30); -- [AI] 내부 처리용 변수
    
    LVS_MARKING_CONDITION          VARCHAR2 (100); -- [AI] 내부 처리용 변수
    LVS_CARRIER_BARCODE_POSITION   VARCHAR2 (100);   -- HLDS check -- [AI] 내부 처리용 변수
    LVS_CUSTOMER_NAME              VARCHAR2 (100); -- [AI] 내부 처리용 변수
    LVS_SITE_CODE                  VARCHAR2 (100); -- [AI] 내부 처리용 변수
    lvs_location_code              VARCHAR2 (30); -- [AI] 내부 처리용 변수
   
    lvl_count                   NUMBER; -- [AI] 내부 처리용 변수
    lvi_check_result_count      NUMBER; -- [AI] 내부 처리용 변수
    lvi_spi_time_check          NUMBER; -- [AI] 내부 처리용 변수
    lvi_reflow_time_check       NUMBER; -- [AI] 내부 처리용 변수
    lvi_coating_time_check      NUMBER; -- [AI] 내부 처리용 변수
    lvi_xray_check              NUMBER; -- [AI] 내부 처리용 변수
    LVI_CARRIER_SIZE            NUMBER; -- [AI] 내부 처리용 변수
    lvl_jig_status              NUMBER; -- [AI] 내부 처리용 변수
    lvi_check_result_ok_count   NUMBER; -- [AI] 내부 처리용 변수
    lvi_check_result_ng_count   NUMBER; -- [AI] 내부 처리용 변수
    LVI_AOI_REWORK_COUNT        NUMBER; -- [AI] 내부 처리용 변수
    LVI_OK_COUNT                NUMBER; -- [AI] 내부 처리용 변수
    LVI_NG_COUNT                NUMBER; -- [AI] 내부 처리용 변수
    lvl_break_value             NUMBER; -- [AI] 내부 처리용 변수
    lvl_hit_value               NUMBER; -- [AI] 내부 처리용 변수
    lvi_count                   NUMBER; -- [AI] 내부 처리용 변수
    lvl_solder_time_check       NUMBER;  -- [AI] 내부 처리용 변수
    lvl_Elapsed_Time            NUMBER; -- [AI] 내부 처리용 변수
    
    LVS_NG_DATE                 VARCHAR2 (20); -- [AI] 내부 처리용 변수
    
    LVI_REPAIR_COUNT            NUMBER; -- [AI] 내부 처리용 변수
   
    lvs_lcr_lot_no              VARCHAR2(100) ; -- [AI] 내부 처리용 변수
    
    LVS_MIN_VALUE               VARCHAR2 (10); -- [AI] 내부 처리용 변수
    LVS_MAX_VALUE               VARCHAR2 (10); -- [AI] 내부 처리용 변수
    
    LVS_CUR_VALUE               VARCHAR2 (10); -- [AI] 내부 처리용 변수
    
    lvs_repair_result           VARCHAR2 (10); -- [AI] 내부 처리용 변수
    lvs_inspect_handling        VARCHAR2 (10); -- [AI] 내부 처리용 변수
    
    lvs_jig_status              VARCHAR2 (10); -- [AI] 내부 처리용 변수
    lvs_use_status              VARCHAR2 (10);   -- [AI] 내부 처리용 변수
    
    LVS_LINE_MARKING_YN         VARCHAR2 (10); -- [AI] 내부 처리용 변수
   ...
   lvdt_backupblock_check_date DATE; -- [AI] 내부 처리용 변수

   LVS_TENSION_CHECK_YN        VARCHAR2 (1); -- [AI] 내부 처리용 변수
   LVS_PART_NO                 VARCHAR2(30); -- [AI] 내부 처리용 변수
   
   lvs_run_no                  VARCHAR2(30); -- [AI] 내부 처리용 변수
   lvs_pid_run_no              VARCHAR2(30); -- [AI] 내부 처리용 변수
   LVS_MARKING_YN              VARCHAR2(30); -- [AI] 내부 처리용 변수
   
   lvl_qc_count                number ; -- [AI] 내부 처리용 변수
   
   lvdt_max_ng_date            date ; -- [AI] 내부 처리용 변수
   lvs_max_spi_date            varchar2(20) ; -- [AI] 내부 처리용 변수
   
   lvl_max_count               number; -- [AI] 내부 처리용 변수
   
   LVS_CARRIER_BARCODE_YN      VARCHAR2 (1); -- [AI] 내부 처리용 변수
   LVS_BOT_BARCODE             VARCHAR2 (50); -- [AI] 내부 처리용 변수
   
   lvs_pcb_tb_suffix           VARCHAR2 (50);   -- [AI] 내부 처리용 변수
   
   lvl_pcb_input_qty           number; -- [AI] 내부 처리용 변수
   lvl_lot_input_qty           number; -- [AI] 내부 처리용 변수
   
   LVDT_BOT_AOI_TIMNE          DATE; -- [AI] 내부 처리용 변수
   LVDT_TOP_AOI_TIMNE          DATE; -- [AI] 내부 처리용 변수
...
*/
   ...
   --------------------------------------------------------------------------
   -- BOT 이후 TOP 이후 경과시간 확인 168 시간 초과시 NG
   --------------------------------------------------------------------------   
   
   IF p_type = 'SMD_BOT_TOP_TIME_CHECK' THEN
     
      if ( substr(p_serial_no, -1) = 'T' ) then  -- BOT 생산 후 TOP 생산
      ...
      ---------------------------------------------------
          ...
          select count(*)
            into lvl_count
            from ip_product_model_master
           where customer_code = 'IP'
             and model_name in (
                                 select model_name
                                   from ip_product_2d_barcode  x
                                  where serial_no = p_serial_no
                               );
      ...
      ---------------------------------------------------------                   
      -- BOT 에서 TOP 투입까지 경과시간 확인, 168 이상이면 NG
      ---------------------------------------------------------
      
               select count(*), max( ( sysdate - to_date ( inspect_date, 'YYYY/MM/DD HH24:MI:SS') ) * ( 24 ) )  -- hour 표기
                 into lvl_count, lvl_Elapsed_Time
                 from IQ_MACHINE_INSPECT_DATA_AOI 
                where pid = substr(p_serial_no, 1, length( p_serial_no ) -1 ) || 'B'    
                  and inspect_date = (
                                      select min(inspect_date)
                                         from IQ_MACHINE_INSPECT_DATA_AOI 
                                        where pid = substr(p_serial_no, 1, length( p_serial_no ) -1 ) || 'B'     
                                     ) ;
               ...
               if ( lvl_count > 0 ) then
                     ...
                     if ( lvl_Elapsed_Time > 168 ) then   -- 7일(168시간 ) 이내 투입해야 함
                       
                          p_result     := 'NG';
                          p_message    := 'SMD_BOT_TOP_TIME_CHECK';
                          p_ng_message := 'Over 168 Hours, BOT > TOP Input:';
                          p_ok_message := 'lvl_Elapsed_Time < 168 hour';                 
                     ...
                     p_result     := 'NG';
                     p_message    := 'ASM_BOT_TOP_TIME_CHECK';
                     p_ng_message := 'No AOI Inspection history found (BOT)';
                     p_ok_message := '';                   
   ...
   IF p_type = 'ASM_BOT_TOP_TIME_CHECK' THEN
      ...
      ---------------------------------------------------------
               ...
               select count(*), MIN ( to_date ( inspect_date, 'YYYY/MM/DD HH24:MI:SS') )                
                 into lvl_count, LVDT_BOT_AOI_TIMNE
                 from IQ_MACHINE_INSPECT_DATA_AOI 
                where pid = substr(p_serial_no, 1, length( p_serial_no ) -1 ) || 'B'    
                  and inspect_date = (
                                      select min(inspect_date)
               ...
               if ( lvl_count > 0 ) then
                     ...
                     select count(*), MIN ( to_date ( inspect_date, 'YYYY/MM/DD HH24:MI:SS') )                
                       into lvl_count, LVDT_TOP_AOI_TIMNE
                       from IQ_MACHINE_INSPECT_DATA_AOI 
                      where pid = p_serial_no
                        and inspect_date = (
                                            select min(inspect_date)
                      ...
                      if ( lvl_count > 0 ) then
                           ...
                           if ( ( LVDT_TOP_AOI_TIMNE - LVDT_BOT_AOI_TIMNE ) * 24 > 168 ) then
                           ...
                           p_result     := 'NG';
                           p_message    := 'ASM_BOT_TOP_TIME_CHECK';
                           p_ng_message := 'No AOI Inspection history found (TOP)';
                           p_ok_message := 'P';                          

CREATE OR REPLACE TRIGGER TRG_IQ_INTERLOCK_CHK_COND_DEL
  /* ================================================================
   * 트리거명  : TRG_IQ_INTERLOCK_CHK_COND_DEL
   * 작성자  : 지성솔루션컨설팅
   * 작성일  : 2026-07-02
   * 수정이력:
   ...
   * ================================================================
   * [AI 분석] 기능 설명:
   *   IQ_INTERLOCK_CHECK_CONDITION 테이블의 DELETE 이벤트 발생 시 원본 트리거 로직에 정의된 후속 처리를 자동 수행한다.
   ...
   * [AI 분석] 복잡도:
   *   조건 분기: IF 0회 / 반복문: 0회
   *   DML: SELECT 1회, INSERT 1회, UPDATE 1회, DELETE 2회
   * ================================================================
   * 검증 방법:
   *   SELECT status FROM user_objects WHERE object_type = 'TRIGGER' AND object_name = 'TRG_IQ_INTERLOCK_CHK_COND_DEL';
   *   SELECT * FROM user_errors WHERE type = 'TRIGGER' AND name = 'TRG_IQ_INTERLOCK_CHK_COND_DEL';

CREATE OR REPLACE PROCEDURE p_check_baking_scan_user (
  /* ================================================================
   * 프로시저명  : P_CHECK_BAKING_SCAN_USER
   * 작성자  : 지성솔루션컨설팅
   * 작성일  : 2020-11-13
   * 수정이력:
...
BEGIN
  ...
  IF ( lvi_count = 0 ) THEN
   ...
   ------------------------------------------------------------------------
    ...
    select count(*), max(line_code), max(model_name)
      INTO lvi_count, lvs_line_code, lvs_model_name

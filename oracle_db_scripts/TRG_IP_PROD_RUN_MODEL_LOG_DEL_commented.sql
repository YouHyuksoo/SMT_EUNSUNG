CREATE OR REPLACE TRIGGER "INFINITY21_JSMES"
  /* ================================================================
   * 트리거명  : TRG_IP_PROD_RUN_MODEL_LOG_DEL
   * 작성자  : 지성솔루션컨설팅
   * 작성일  : 2026-07-02
   * 수정이력:
   ...
   * ================================================================
   * [AI 분석] 기능 설명:
   *   IP_PRODUCT_RUN_MODEL 테이블의 DELETE 발생 시 변경 이력 또는 감사 로그를 자동 기록한다.
   ...
   * [AI 분석] 복잡도:
   *   조건 분기: IF 0회 / 반복문: 0회
   *   DML: SELECT 1회, INSERT 1회, DELETE 3회
   * ================================================================
   * 검증 방법:
   *   SELECT status FROM user_objects WHERE object_type = 'TRIGGER' AND object_name = 'TRG_IP_PROD_RUN_MODEL_LOG_DEL';
   *   SELECT * FROM user_errors WHERE type = 'TRIGGER' AND name = 'TRG_IP_PROD_RUN_MODEL_LOG_DEL';
   *   주의: 검증 목적으로 대상 테이블에 DML을 실행하지 않는다.
   * ================================================================ */
."TRG_IP_PROD_RUN_MODEL_LOG_DEL" 
  before delete on IP_PRODUCT_RUN_MODEL
  for each row
declare
  -- local variables here
begin

          -- Interlock condition delete ？？？？？？？

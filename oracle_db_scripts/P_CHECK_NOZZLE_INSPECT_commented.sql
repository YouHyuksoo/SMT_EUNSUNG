CREATE OR REPLACE PROCEDURE P_CHECK_NOZZLE_INSPECT (
  /* ================================================================
   * 프로시저명  : P_CHECK_NOZZLE_INSPECT
   * 작성자  : 지성솔루션컨설팅
   * 작성일  : 2020-07-30
   * 수정이력:
   ...
   -- [AI] 예외 상황을 원본 처리 방식대로 처리한다.
      ...
      ELSE      
          ...
          UPDATE IP_PRODUCT_LINE
             SET NOZZLE_CHECK_STATUS   = substr(p_result,1,1) ,
                 NOZZLE_CHECK_DATE     = sysdate
           WHERE line_code = SUBSTR(p_line_code, 1, 2);         
...
EXCEPTION
   ...
   WHEN OTHERS THEN
        rollback;
        p_return := 'NG, [P_CHECK_NOZZLE_INSPECT] '
                    || 'Phase=' 
                    || phase
                    || ', '
                    || SQLERRM;

CREATE OR REPLACE PROCEDURE P_INSERT_SPI_PEMTRON_RAW(
  /* ================================================================
   * 프로시저명  : P_INSERT_SPI_PEMTRON_RAW
   * 작성자  : 지성솔루션컨설팅
   * 작성일  : 2020-10-14
   * 수정이력:
   *   2020-10-14 - 지성솔루션컨설팅 - 최초 작성
...
IS
   ...
   lvs_data_columns   VARCHAR2 (20)  := ''; -- [AI] 내부 처리용 변수
...
BEGIN
   ...
   lvs_data_columns := to_char( p_data.LAST );
...
EXCEPTION
   ...
   WHEN OTHERS THEN
        ...
        LVS_ERRORMSG := '[P_INSERT_SPI_PEMTRON_RAW]' || ' ' || SUBSTR (SQLERRM, 1, 300);
        ...
        INSERT INTO ICOM_MACHINE_INSERT_LOG(
                                    ...
                                    VALUES (
                                             SYSDATE,
                                             LVS_ERRORMSG  ,
                                             'FILE = '||lvs_file_name||' LINE='||lvs_line_code,

CREATE OR REPLACE PROCEDURE "P_PRODUCT_FG_ISSUE_DATE" (
  /* ================================================================
   * 프로시저명  : P_PRODUCT_FG_ISSUE_DATE
   * 작성자  : 지성솔루션컨설팅
   * 작성일  : 2017-08-07
   * 수정이력:
   ...
   *   P_ISSUE_DATE - 원본 선언부 기준 입력/출력 파라미터
         ...
         /******************************************/
           ...
           ***********************************/
           ...
           insert into ip_product_fg_issue ( 
              ...
              to_date(p_issue_date, 'YYYY/MM/DD'),    -- sysdate, 
...
--           where barcode = c01.barcode ; 
           ...
           ************************************************************/
           UPDATE ip_product_2d_barcode X
              SET X.SHIPPING_DATE    = to_date(p_issue_date, 'YYYY/MM/DD') ,   -- SYSDATE, 
      ...
      begin
        ...
        select txn_deficit, issue_date, issue_sequence , barcode,       qty
         ...
         where x.barcode            = p_barcode 
           and x.last_modify_date = ( 
                                  select max(y.last_modify_date) 

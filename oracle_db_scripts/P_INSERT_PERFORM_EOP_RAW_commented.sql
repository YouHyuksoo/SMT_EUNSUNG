CREATE OR REPLACE PROCEDURE P_INSERT_PERFORM_EOP_RAW (
  /* ================================================================
   * 프로시저명  : P_INSERT_PERFORM_EOP_RAW
   * 작성자  : 지성솔루션컨설팅
   * 작성일  : 2021-02-17
   * 수정이력:
   ...
   *   IQ_MACHINE_INSPECT_DATA_EOP - 원본 로직 참조 테이블
   * ================================================================
   * [AI 분석] 호출 객체:
   *   F_GET_RUN_NO_BY_PID
   *   P_DATA
...
BEGIN
   ...
   lvs_column_count :=  to_char( p_data.LAST ); 
       ...
       INSERT INTO IQ_MACHINE_INSPECT_DATA_EOP (      
...
EXCEPTION
   ...
   WHEN OTHERS THEN
        ...
        LVS_ERRORMSG := '[P_INSERT_PERFORM_EOP_RAW]' || lvs_sql || SUBSTR (SQLERRM, 1, 200);

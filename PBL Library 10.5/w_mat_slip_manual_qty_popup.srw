HA$PBExportHeader$w_mat_slip_manual_qty_popup.srw
$PBExportComments$Purchase Order Lot Divide Popup
forward
global type w_mat_slip_manual_qty_popup from w_none_dw_popup_root
end type
type mle_log from multilineedit within w_mat_slip_manual_qty_popup
end type
type st_status from statictext within w_mat_slip_manual_qty_popup
end type
type dw_1 from so_datawindow within w_mat_slip_manual_qty_popup
end type
type st_1 from so_statictext within w_mat_slip_manual_qty_popup
end type
type em_total_qty from so_editmask within w_mat_slip_manual_qty_popup
end type
type pb_ok from so_commandbutton within w_mat_slip_manual_qty_popup
end type
type st_2 from so_statictext within w_mat_slip_manual_qty_popup
end type
type em_divide_qty from so_editmask within w_mat_slip_manual_qty_popup
end type
type pb_1 from so_commandbutton within w_mat_slip_manual_qty_popup
end type
end forward
...
global w_mat_slip_manual_qty_popup w_mat_slip_manual_qty_popup

type variables
long lvl_row
end variables

on w_mat_slip_manual_qty_popup.create
int iCurrent
call super::create
...
on w_mat_slip_manual_qty_popup.destroy
call super::destroy
...
type p_title from w_none_dw_popup_root`p_title within w_mat_slip_manual_qty_popup
integer width = 2807
end type

type cb_close from w_none_dw_popup_root`cb_close within w_mat_slip_manual_qty_popup
...
type st_msg from w_none_dw_popup_root`st_msg within w_mat_slip_manual_qty_popup
boolean visible = true
integer y = 0
end type
	...
	i++
	 if long( dw_1.object.qty[i])  = 0 or isnull(dw_1.object.qty[i] )  then 
		...
		lvl_total = lvl_total +  long( dw_1.object.qty[i])
	     w_mat_receipt_slip_master.ivl_divide_qty[j] = long( dw_1.object.qty[i])
	 end if 

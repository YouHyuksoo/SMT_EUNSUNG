HA$PBExportHeader$uo_ymdh_now_calendar.sru
$PBExportComments$yyyy mm dd Calendar Today
forward
global type uo_ymdh_now_calendar from userobject
end type
type em_2 from editmask within uo_ymdh_now_calendar
end type
type cb_first from commandbutton within uo_ymdh_now_calendar
end type
type cb_1 from commandbutton within uo_ymdh_now_calendar
end type
type em_yyyymm from editmask within uo_ymdh_now_calendar
end type
type hsb_1 from hscrollbar within uo_ymdh_now_calendar
end type
type em_1 from editmask within uo_ymdh_now_calendar
end type
type dw_1 from datawindow within uo_ymdh_now_calendar
end type
end forward
...
global uo_ymdh_now_calendar uo_ymdh_now_calendar
...
public function datetime ymdtext ()
public subroutine settime (string arg_time)
end prototypes
...
public subroutine uf_small ();WIDTH  = EM_1.WIDTH  + cb_first.width + EM_2.WIDth
HEIGHT = EM_1.HEIGHT
...
public function datetime text ();RETURN DATETIME(DATE(EM_1.TEXT) , TIME(EM_2.TEXT) )
end function
...
public function datetime ymdtext ();RETURN DATETIME(DATE(EM_1.TEXT) )
end function

public subroutine settime (string arg_time);em_2.text = arg_time


end subroutine

on uo_ymdh_now_calendar.create
...
on uo_ymdh_now_calendar.destroy
destroy(this.em_2)
...
event constructor;SELECT TO_CHAR(SYSDATE , 'YYYY/MM/DD')  , TO_CHAR(SYSDATE , 'HH24') INTO :text , :em_2.text
FROM DUAL ;
end event

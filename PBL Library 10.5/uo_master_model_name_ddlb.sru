HA$PBExportHeader$uo_master_model_name_ddlb.sru
$PBExportComments$Item Code
forward
global type uo_master_model_name_ddlb from dropdownlistbox
end type
end forward
...
global uo_master_model_name_ddlb uo_master_model_name_ddlb
...
DECLARE CUR_01 CURSOR FOR 
  SELECT DISTINCT MASTER_MODEL_NAME	  
    FROM IP_PRODUCT_MODEL_MASTER  
   WHERE  MASTER_MODEL_NAME <> '*'
...
DO 
	...
	IF F_SQL_CHECK_WITH_MSG('SELECT FROM MODEL MASTER') < 0 THEN 
		CLOSE CUR_01 ;
		RETURN 0
	END IF	
...
on uo_master_model_name_ddlb.create
end on

on uo_master_model_name_ddlb.destroy
end on

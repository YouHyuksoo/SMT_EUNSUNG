HA$PBExportHeader$uo_line_code_smt_dd.sru
$PBExportComments$Line ddlb Object
forward
global type uo_line_code_smt_dd from dropdownlistbox
end type
end forward
...
global uo_line_code_smt_dd uo_line_code_smt_dd
...
//RETURN TRIM(MID( THIS.TEXT ,  POS( THIS.TEXT , ':' ) +1 , 1  ))
end function
...
event constructor;LONG I, LVL_MES_DISPLAY_SEQUENCE
STRING LVS_LINE_CODE , LVS_LINE_NAME , LVS_LINE_CODE_CONDITION

DECLARE CUR_01 CURSOR FOR 
	SELECT LINE_CODE, LINE_NAME, MES_DISPLAY_SEQUENCE
  	  FROM  IP_PRODUCT_LINE
	 WHERE ORGANIZATION_ID = :GVI_ORGANIZATION_ID 
	      AND LINE_DIVISION = 'D'
	      AND LINE_CODE <> '*'
...
DO 
	FETCH CUR_01 INTO :LVS_LINE_CODE , :LVS_LINE_NAME, :LVL_MES_DISPLAY_SEQUENCE;
...
on uo_line_code_smt_dd.create
end on

on uo_line_code_smt_dd.destroy
end on

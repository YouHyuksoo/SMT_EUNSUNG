HA$PBExportHeader$w_mat_purchase_order_master.srw
$PBExportComments$Material Purchase Order Master
forward
global type w_mat_purchase_order_master from w_main_root
end type
type st_1 from so_statictext within w_mat_purchase_order_master
end type
type ddlb_item_code from uo_item_code within w_mat_purchase_order_master
end type
type st_3 from so_statictext within w_mat_purchase_order_master
end type
type uo_dateset from uo_ymd_calendar within w_mat_purchase_order_master
end type
type rb_lst from so_radiobutton within w_mat_purchase_order_master
end type
type rb_hst from so_radiobutton within w_mat_purchase_order_master
end type
type st_4 from so_statictext within w_mat_purchase_order_master
end type
type ddlb_supplier_code from uo_supplier_code within w_mat_purchase_order_master
end type
type sle_item_name from so_singlelineedit within w_mat_purchase_order_master
end type
type st_14 from so_statictext within w_mat_purchase_order_master
end type
type sle_1 from so_singlelineedit within w_mat_purchase_order_master
end type
type st_5 from so_statictext within w_mat_purchase_order_master
end type
type uo_dateend from uo_ymdend_calendar within w_mat_purchase_order_master
end type
type tab_1 from tab within w_mat_purchase_order_master
end type
...
type ddlb_language from uo_basecode within tabpage_4
end type
...
type sle_order_group_no from so_singlelineedit within w_mat_purchase_order_master
end type
type st_7 from so_statictext within w_mat_purchase_order_master
end type
type cbx_show_all from so_checkbox within w_mat_purchase_order_master
end type
type ddlb_order_type from uo_basecode within w_mat_purchase_order_master
end type
type st_8 from so_statictext within w_mat_purchase_order_master
end type
type gb_1 from so_groupbox within w_mat_purchase_order_master
end type
type gb_5 from so_groupbox within w_mat_purchase_order_master
end type
end forward
...
global w_mat_purchase_order_master w_mat_purchase_order_master

type variables
string ivs_preview_yn = 'N'
end variables

on w_mat_purchase_order_master.create
int iCurrent
call super::create
...
choose case gvs_ue_data_control
	...
	case 'RETRIEVE'			
		...
		if rb_lst.checked then 
			...
			dw_1.retrieve(ddlb_supplier_code.text+'%' , ddlb_item_code.text() + '%',  uo_dateset.text() , uo_dateend.text(),  sle_order_group_no.text+'%' , lvs_user_id , ddlb_order_type.getcode( )+'%' ,   gvi_organization_id)
		else
			dw_3.retrieve(ddlb_supplier_code.text+'%' , ddlb_item_code.text() + '%',  uo_dateset.text() , uo_dateend.text(),   ddlb_order_type.getcode( )+'%' ,  sle_order_group_no.text+'%' , gvi_organization_id)
		end if
...
//			dw_2.object.material_mfs[row] = lvs_date
...
long j
	...
	for li_index = dw_1.rowcount() to 1 step -1
		
				if dw_1.getitemstring(li_index,'CHECK_YN') = 'N' then continue
						if dw_1.object.arrival_qty[li_index] <> 0 then
						 messagebox('$$HEX2$$668b4a54$$ENDHEX$$','$$HEX9$$a28b5553e89006527065cf91f25dfa51d153$$ENDHEX$$,$$HEX5$$0d4efd802052649601ff$$ENDHEX$$')
						 rollback;
						 RETURN
						 else
						  dw_1.deleterow(li_index)
				end if
       j++
	next	
...
type dw_5 from w_main_root`dw_5 within w_mat_purchase_order_master
integer y = 564
integer height = 1184
end type

type dw_4 from w_main_root`dw_4 within w_mat_purchase_order_master
integer y = 568
integer width = 4544
integer height = 1608
...
type dw_3 from w_main_root`dw_3 within w_mat_purchase_order_master
integer y = 568
integer width = 4544
integer height = 1360
boolean titlebar = true
...
type dw_2 from w_main_root`dw_2 within w_mat_purchase_order_master
integer y = 1928
integer width = 4535
integer height = 724
...
type dw_1 from w_main_root`dw_1 within w_mat_purchase_order_master
integer y = 568
integer width = 4544
integer height = 1360
boolean titlebar = true
string title = "Material Purchase Order List"
string dataobject = "d_mat_purchase_order_lst"
end type
...
type uo_tabpages from w_main_root`uo_tabpages within w_mat_purchase_order_master
end type
...
event clicked;call super::clicked;open(w_mat_item_purchase_excel_form_popup)
end event
...
integer height = 112
integer taborder = 90
string text = "Show Unit Price"
end type

event clicked;call super::clicked;openwithparm(w_mat_unit_price_4_purchase_order_popup , dw_2)
...
integer taborder = 50
boolean bringtotop = true
string text = "Safety Inventory"
end type

event clicked;call super::clicked;open(w_mat_item_safety_inventory_popup)
end event
...
//	messagebox('Notify','The arrival qty of this record is found,so it can not be performed!')
//	return 
	lvl_origin_qty =  dw_1.object.order_qty[dw_1.getrow()] - dw_1.object.arrival_qty[dw_1.getrow()]
		openwithparm(w_mat_order_qty_divide_popup, lvl_origin_qty)
		if gst_return.gvb_return = false then 
			return
		else 	
		...
		dw_1.rowscopy(j, j, Primary!, dw_1, j+1 , Primary!)
		dw_1.object.order_qty[j] = lvl_arrival_qty
	    dw_1.object.arrival_qty[j] = lvl_arrival_qty
		dw_1.object.order_amt[j] = dw_1.object.order_qty[j] * dw_1.object.unit_price[j]
		
		dw_1.object.order_qty[j + 1]  =lvl_new_qty
		dw_1.object.arrival_qty[j+1] = 0
...
this.ddlb_language=create ddlb_language
...
if cbx_show_price.checked = false then 
	dw_4.object.unit_price.color = rgb(255,255,255)
	dw_4.object.purchase_amount.color = rgb(255,255,255)
	dw_4.object.purchase_amount_total.color = rgb(255,255,255)
else
	dw_4.object.unit_price.color = 0
	dw_4.object.purchase_amount.color = 0
	dw_4.object.purchase_amount_total.color = 0
end if
...
event selectionchanged;call super::selectionchanged;f_dual_lang_change_dwtext_dynamic( dw_4 , this.getcode( ) )
end event

...
Require administrator execution and explicit parameters. Create `eunsung-deploy` only if absent, never print its generated password, deny administrator-group membership, create `.deploy/{incoming,releases,shared,logs,scripts}`, and grant Modify only to `.deploy`. Register the supplied public key in that account's `.ssh\authorized_keys` with Windows ACLs. Validate read/execute access to Node and Oracle Client. Install exactly `pnpm@10.28.1` and `pm2@6.0.6` for the deployment account only when absent.

For reboot restoration use built-in Task Scheduler, not an unpinned PM2 service package. Create a reviewed `.deploy\scripts\Resurrect-EunsungPm2.ps1` wrapper that fixes `USERPROFILE` and `PM2_HOME`, then calls the pinned PM2 command and checks its exit code. Register `EunsungMES-PM2-Resurrect` with an `AtStartup` trigger, `eunsung-deploy` principal, `LogonType S4U`, and limited run level. Verify with `Start-ScheduledTask`, bounded status/process checks, and `Stop-ScheduledTask`; do not reboot. Rollback unregisters only this exact task and removes only the generated wrapper.

- [ ] **Step 2: Add bootstrap safety assertions**

Assert no hard-coded credential, no administrator-group addition, scoped ACL paths, idempotent account/path/task checks, exact scheduled-task name, S4U/limited principal, checked PM2 exit, task rollback, and no reboot/shutdown command.

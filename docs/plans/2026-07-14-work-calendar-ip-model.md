...
- Delete: `apps/backend/src/utils/shift-resolver.ts` (**Task 3에서 발견**: 삭제된 `ShiftPattern` 엔티티를 참조하는 고아 파일. 어떤 파일도 import하지 않아 tsc가 잡지 못한다.)
...
- [ ] **Step 6: 고아 shift-pattern 잔재 삭제**

```bash
rm apps/backend/src/modules/master/services/shift-pattern.service.ts
rm apps/backend/src/utils/shift-resolver.ts
```

`shift-resolver.ts`는 삭제된 `ShiftPattern` 엔티티를 참조하지만 어떤 파일도 import하지 않는 고아라 tsc가 잡지 못한다. 삭제 후 `grep -r "ShiftResolver\|shift-resolver" apps/backend/src`가 0건인지 확인한다.

...
- 배포 루트: `D:\Project\SMT_EUNSUNG\.deploy`
...
PR 및 fork 이벤트에서는 배포 job을 실행하지 않는다. 동시 배포는 workflow concurrency로 직렬화한다. workflow 권한은 `contents: read`로 제한하고, 외부 Action은 검토된 commit SHA로 고정한다. `known_hosts`는 GitHub Secret의 host key로 만들고 `StrictHostKeyChecking=yes`를 강제한다. `jisung-development` GitHub Environment를 필수로 사용하고 배포 브랜치를 `main`으로 제한한다. 저장소 `main` 보호 규칙도 적용하며 로그에는 secret 값을 출력하지 않는다.

## 서버 전용 설정

다음 파일은 Git에 포함하지 않고 서버에만 유지한다.

- `D:\Project\SMT_EUNSUNG\.deploy\shared\backend.env`: 최초 구성 시 현재 로컬 파일을 안전하게 복사한다.
- `D:\Project\SMT_EUNSUNG\.deploy\shared\frontend-database.json`: 기존 서버 설정을 최초 구성 시 복사한다.
...
기존 `D:\Project\SMT_EUNSUNG` 작업 트리의 tracked 파일은 수정하거나 배포 입력으로 사용하지 않는다. 따라서 현재 존재하는 `sw.js`, 로그, `package-lock.json` 변경도 덮어쓰지 않는다. GitHub에서 받은 정확한 커밋을 `D:\Project\SMT_EUNSUNG\.deploy\releases\<sha>`에 새로 풀어 기존 실행 산출물과 빌드를 격리한다. `.deploy/`는 저장소 `.gitignore`에 등록한다.
...
- release 경로 또는 shared 설정 파일 검증 실패: 즉시 실패
...
2. 비관리자 `eunsung-deploy` 계정을 생성하고 `.deploy` 경로에만 필요한 ACL을 부여한다.
3. 공개키를 해당 계정의 `authorized_keys`에 등록하고 Windows OpenSSH 요구 ACL을 적용한다.
4. 키 인증과 SSH host key를 검증한다.
5. `jisung-development` Environment를 만들고 배포 브랜치를 `main`으로 제한한다.
6. Environment Secrets에 host, port, user, private key, host key를 등록한다.
7. 서버 shared 영역에 백엔드 `.env`와 프론트 DB 설정을 배치한다.
8. 비대화형 SSH에서 Node, Git, pnpm, PM2, Oracle Client 경로와 버전을 검증한다.
9. workflow와 PM2 설정을 반영한 뒤 수동 배포로 최초 검증한다.
10. 최초 성공 후 `main` push 자동 배포를 확인한다.

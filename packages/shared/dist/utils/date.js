...
exports.getDateTimePrefix = getDateTimePrefix;
...
exports.getShiftCode = getShiftCode;
 ...
 */
function getDateTimePrefix(date = new Date()) {
 ...
 * // 밤 12시 -> 'NIGHT'
 */
function getShiftCode(date = new Date()) {
 ...
 */
function getRelativeTime(date, lang = 'ko') {
    const d = typeof date === 'string' ? new Date(date) : date;
    const now = new Date();
    ...
    if (diffSec < 60)
        return l.justNow;
    if (diffMin < 60)
        return `${diffMin}${l.minute}`;
    if (diffHour < 24)
        return `${diffHour}${l.hour}`;

...
import { buildConnectString } from '@/lib/connect-string';

// connectString 생성은 connect-string.ts 단일 소스에서 관리한다.
// 클라이언트(DatabasePanel 미리보기)와 동일한 로직을 쓰도록 재export.
export { buildConnectString };

/**
 * Oracle thick client 초기화.
 * thin mode는 Oracle 12.1+ 만 지원하므로, 11g 등 구버전 DB 접속에는 thick mode가 필수다.
 * (thin mode로 11g 접속 시 NJS-138 발생)
 * 탐색 우선순위:
 *   1) ORACLE_CLIENT_LIB_DIR (.env.production 등으로 서버별 지정)
 *   2) 개발 PC 기본 경로
 *   3) 시스템 PATH의 Oracle Client 자동 탐색 (ORACLE_HOME\bin 등)
 * 어느 모드로 동작하는지 로그로 남겨 thin 폴백을 진단 가능하게 한다.
 */
function initOracleThick(): void {
  const explicitPaths = [
  ...
  for (const libDir of explicitPaths) {
    ...
    try {
      ...
      console.warn(`[db] thick 초기화 실패 (libDir=${libDir}): ${message}`);
    }
  }
  ...
  try {
    ...
    if (message.includes('already been initialized')) return;
    console.warn(
      `[db] Oracle thick client를 찾지 못해 thin mode로 동작합니다. ` +
        `11g 등 구버전 DB 접속 시 NJS-138이 발생하니 ORACLE_CLIENT_LIB_DIR에 ` +
        `Oracle Client 경로를 지정하세요. (${message})`,
    );
  }
}
...
// BLOB 컬럼을 자동으로 Buffer 로 변환 (Lob 객체 반환 방지 → 바이너리 직접 사용 가능)

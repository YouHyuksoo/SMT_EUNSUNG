...
/**
 * 시프트별 5타임 구간 정보 — 모든 경계가 이어지는 연속 라벨.
 * SQL CASE가 10분 휴식 시간을 각 구간에 흡수해 집계하므로(BETWEEN으로 연속),
 * 표시 라벨도 경계가 서로 맞닿도록 구성한다. (근무 전체 12h = 5구간 합계와 일치)
 *   주간: A(08:00~10:10)·B(10:10~13:00)·C(13:00~15:10)·D(15:10~17:30)·E(17:30~20:00)
 *   야간: A(20:00~22:10)·B(22:10~01:00)·C(01:00~03:10)·D(03:10~05:30)·E(05:30~08:00)
 */
export const SHIFT_ZONES: Record<string, { zone: string; label: string }[]> = {
  A: [
    { zone: 'A', label: '08:00~10:10' },
    { zone: 'B', label: '10:10~13:00' },
    { zone: 'C', label: '13:00~15:10' },
    { zone: 'D', label: '15:10~17:30' },
    { zone: 'E', label: '17:30~20:00' },
  ],
  B: [
    { zone: 'A', label: '20:00~22:10' },
    { zone: 'B', label: '22:10~01:00' },
    { zone: 'C', label: '01:00~03:10' },
    { zone: 'D', label: '03:10~05:30' },
    { zone: 'E', label: '05:30~08:00' },
  ],
};

/* ------------------------------------------------------------------ */
/*  IO_DATE 시간 기준:                                                  */
/*  DB 서버가 베트남 현지(ICT, UTC+7)에서 돌고 IO_DATE 도 동일 타임존   */
/*  으로 저장되므로 별도 보정 없이 SYSDATE 와 동일 기준으로 비교한다.   */
/*  한국에서 접속해도 SYSDATE 자체가 베트남 시간이라 보정 불필요.       */
/*  (시간대가 다른 사이트로 이식 시 여기만 조정.)                       */
/* ------------------------------------------------------------------ */
const IO_LOCAL = `IO_DATE`;
...
SELECT t.PLAN_DATE, t.LINE_CODE,
       ...
       t.SHIFT_CODE, t.PLAN_QTY, l.UPH_VALUE AS UPH, l.MODEL_NAME, l.ITEM_CODE,
       t.WORKER_QTY, NVL(t.COMMENTS, l.COMMENTS) AS COMMENTS,
       CAST(NULL AS VARCHAR2(50)) AS LEADER_ID,
       CAST(NULL AS VARCHAR2(50)) AS SUB_LEADER_ID,
       CAST(NULL AS VARCHAR2(100)) AS LEADER_NAME,
       CAST(NULL AS VARCHAR2(100)) AS SUB_LEADER_NAME
  FROM IP_PRODUCT_LINE_TARGET t
  LEFT JOIN IP_PRODUCT_LINE l
    ON l.LINE_CODE = t.LINE_CODE

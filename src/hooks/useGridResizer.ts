...
export function useGridResizer(storageKey: string, initialWidths: (number | string)[]) {
  ...
  // hydration 가드: localStorage 로드가 끝나기 전에는 저장 effect가 INITIAL 값으로 덮어쓰지 못하게 막음.
  // ref 대신 state — strict mode에서 effect가 같은 closure로 두 번 실행될 때도 안전하게 skip 보장.
  ...
  // 단, 로드 effect가 끝나기 전(첫 렌더)에는 INITIAL_WIDTHS로 덮어쓰지 않도록 isHydrated 가드.
  useEffect(() => {
    if (!isHydrated) return;
    if (typeof window !== 'undefined') {
      localStorage.setItem(storageKey, JSON.stringify(widths));
    }
  }, [storageKey, widths, isHydrated]);
